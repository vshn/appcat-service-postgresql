...
const (
	errNotMyType    = "managed resource is not the expected custom resource"
...
func Setup(mgr ctrl.Manager, o controller.Options) error {
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(v1alpha1.PostgresStandaloneGroupVersionKind),
		managed.WithExternalConnecter(&connector{
			kube:         mgr.GetClient(),
			usage:        resource.NewProviderConfigUsageTracker(mgr.GetClient(), &providerv1alpha1.ProviderConfigUsage{}),
			newServiceFn: newNoOpService}),
		managed.WithLogger(logging.NewLogrLogger(o.Log.WithValues("controller", name))),
